...
class Elmo{
  ...
  {
    chdir(__DIR__);
    if ($route) $route = str_replace(".php", "", $route);
    if (!$route) $route = str_replace(['index.php'], "", $_GET['q']);
  ...
  {
    if (!is_dir(dirname($location))) mkdir(dirname($location));
...
if (php_sapi_name() == 'cli') Elmo::bundle();